...
mvcNotify.autoClearTimeout = 4000; //set in milliseconds
mvcNotify.elementsDontClearOnClick = ["a", ":button", ":submit"]; //on click of these elements, don't clear messages with fade
mvcNotify.typesToAutoClear = "success"; //if mvcNotify.autoClearTimeout > 0, auto clear these messages. Can be string like "all" or "success" or string array like ["success", "warning"].
...
mvcNotify.displayMessage = function (message, messageType) {
    ...
    strHtml = '<div class="messagebox ' + messageType.toLowerCase() + '">' + (messageType.toLowerCase() == "success" ? '<i class="fa fa-check"></i> ' : '<i class="fa fa-close"></i> ') + message + '</div>';
...
mvcNotify.handleAjaxMessages = function () {
    $(document).ajaxSuccess(function (event, request) {
        var msg = request.getResponseHeader('X-showErrorInPopup');
        if (msg != null)
        {
            if (msg.toLowerCase() == "true") {
                mvcNotify.displayErrorInModal(request);
            }
            else {
...
mvcNotify.displayErrorInModal = function (request) {
    $('#modalErrorMessage').empty();
    var msg = request.getResponseHeader('X-Message');
    if (msg) {
        $('#modalErrorMessage').append('<div class="alert alert-danger" role="alert">' + msg + '</div>');
    }
},
